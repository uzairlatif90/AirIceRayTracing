...
void RunMultiRayCode_loop(){
  ...
  // TH2D *h2b=new TH2D("","",TotalStepsHb+200,GridStartHb-1,GridStopHb+1,TotalStepsThb+200,GridStartThb-1,GridStopThb+1);
  // TH2D *h2c=new TH2D("","",TotalStepsHb+200,GridStartHb-1,GridStopHb+1,TotalStepsThb+200,GridStartThb-1,GridStopThb+1);
  // TH2D *h2corr=new TH2D("","",TotalStepsHb+200,GridStartHb-1,GridStopHb+1,TotalStepsThb+200,GridStartThb-1,GridStopThb+1);

  TH2D *h2b=new TH2D("","",100,log(GridStartHb-1),log(GridStopHb+1),100,GridStartThb-1,GridStopThb+1);
  TH2D *h2c=new TH2D("","",100,log(GridStartHb-1),log(GridStopHb+1),100,GridStartThb-1,GridStopThb+1);
  TH2D *h2corr=new TH2D("","",100,log(GridStartHb-1),log(GridStopHb+1),100,GridStartThb-1,GridStopThb+1);
  //TH2D *launchtest=new TH2D("","",100,log(GridStartHb-1),log(GridStopHb+1),100,GridStartThb-1,GridStopThb+1);

  TH1D * h1=new TH1D("","",200,0,2000);
  TH1D * h1error_dRR=new TH1D("","",200,0,100);
  TH1D * h1error=new TH1D("","",200,0,1);
  ...
  for(int ih=0;ih<TotalStepsHb;ih++){
	...
	output.push_back(-1000);
      ...
      int rtParameter=6; //0 is AirTxHeight, 1 is THD, 2 is Optical Path in Ice, 3 is Optical Path in Air, 4 is Launch Angle in Air, 5 is THD Air, 6 is Refl Coeff S, 7 is Refl Coeff For P 
      ...
      if(rtresult!=-1000){
	launchtest->SetPoint(count1,log(hR),thR,output[4]-thR);
	h2b->Fill(log(hR),thR,rtresult);
	gr2b->SetPoint(count1,log(hR),thR,rtresult);
	count1++;
      }
      ...
      if(interresult!=-1000){
	h2c->Fill(log(hR),thR,interresult);
	gr2c->SetPoint(count2,log(hR),thR,interresult);
	count2++;
      }
      
      if(rtresult!=-1000 && interresult!=-1000){
	h1->Fill(Duration);
	h2corr->Fill(log(hR),thR,((rtresult-interresult)/rtresult)*100);
	gr2corr->SetPoint(count3,log(hR),thR,((rtresult-interresult)/rtresult)*100);
	count3++;
  ...
  gr2b->SetTitle("RayTrace results for ReflCoefS; log(Tx Height in Air (m)); Straight Line Angle (m); ReflCoefS ()");
  gr2c->SetTitle("Interpolated results for ReflCoefS; log(Tx Height in Air (m)); Straight Line Angle (m); ReflCoefS ()");
  gr2corr->SetTitle("Percentage Error for ReflCoefS; log(Tx Height in Air (m)); Straight Line Angle (m); |#DeltaReflCoefS|/ReflCoefS x 100");
  h1error->SetTitle("Absolute Error for ReflCoefS; |#DeltaReflCoefS| () ; Cumulative Fraction of Tx Positions;");
  h1error_dRR->SetTitle("Percentage Error for ReflCoefS; |#DeltaReflCoefS|/ReflCoefS x 100 ; Cumulative Fraction of Tx Positions;");
  ...
  c2b->SetGridy();
  gr2c->Draw("cont4z");
  c2b->SaveAs("ReflCoefS_Inter.png");
  ...
  c2c->SetGridy();
  gr2corr->Draw("cont4z");
  c2c->SaveAs("ReflCoefS_RT_IT.png");
  ...
  c2d->SaveAs("ReflCoefS_RT_IT_hist.png");
  ...
  c2e->SetGridy();
  h1->Draw("");
  c2e->SaveAs("ReflCoefS_Duration_hist.png");
