...
int main(int argc, char **argv){
  ...
  if(argc==1){
    ...
    cout<<"Example run command: ./SingleRayAirIceRefraction 200 170 20000 3000"<<endl;
    cout<<"Here 200 is Antenna Depth in m, 170 is the ray launch angle (where is 0 vertically up) in deg, 20000 is the Tx Height in m and 3000 is Ice Layer Height in m"<<endl;
    return 0;
  }
  if(argc<5){
  ...
  if(argc==5){
    cout<<"Antenna Depth is set at "<<atof(argv[1])<<" m, The Ray Launch Angle is set at "<<atof(argv[2])<<" deg, Tx Height is set at "<<atof(argv[3]) <<" m, Ice Layer Height is set as "<<atof(argv[4])<<" m"<<endl;
  } 
  if(argc>5){
  ...
  double TxHeight=atof(argv[3]);//20000;////Height of the source
  ...
  for(int il=0;il<MaxLayers-SkipLayersAbove-SkipLayersBelow;il++){
    ...
    for(double i=LayerStartHeight;i>LayerStopHeight;i--){
      ...
      //cout<<ipoints<<" "<<Refracted_x<<" "<<i<<endl;
      ////Path of the straight line ray
      //cout<<ipoints<<" "<<Refracted_x<<" "<<StraightLine_y<<endl;

      ////To make sure that the residual (the difference of the straight path from the curved path) in air is only caculated above the ice surface
      if(StraightLine_y>=IceLayerHeight){
	//cout<<ipoints<<" "<<Refracted_x<<" "<<i-StraightLine_y<<endl;
      }
